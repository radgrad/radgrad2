...
import { AcademicPlan, DescriptionPair } from '../../../typings/radgrad';
...
const descriptionPairs = (plan: AcademicPlan): DescriptionPair[] => [
...
const itemTitleString = (plan: AcademicPlan): string => {
  const slug = Slugs.getNameFromID(plan.slugID);
  return `${plan.name} (${plan.year}) (${slug})`;
};

const itemTitle = (plan: AcademicPlan): React.ReactNode => (
  <React.Fragment>
...
interface AdminDataModelAcademicPlansPageProps extends AdminDataModeMenuProps {
  items: AcademicPlan[];
}
/**
 ...
 * @constructor
 */
const AdminDataModelAcademicPlansPage: React.FC<AdminDataModelAcademicPlansPageProps> = (props) => {
