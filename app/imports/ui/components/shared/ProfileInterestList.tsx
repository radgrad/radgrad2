...
const ProfileInterestList: React.FC<ProfileInterestListProps> = ({ profile, size }) => {
  ...
  const interests = Users.getInterestIDsByType(profile.userID);
  const userInterests = interests[0].map((interestID) => Interests.findDoc(interestID));
  const careerGoalInterests = interests[1].map((interestID) => Interests.findDoc(interestID));
  return (
    <Label.Group size={size}>
      ...
      {careerGoalInterests.map((interest) => {
        const interestSlug = itemToSlugName(interest);
        return (
          <Label as={Link} key={interest._id} to={Router.buildRouteName(match, `/${EXPLORER_TYPE.HOME}/${EXPLORER_TYPE.INTERESTS}/${interestSlug}`)} size={size}>
