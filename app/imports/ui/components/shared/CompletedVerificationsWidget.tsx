...
const CompletedVerificationsWidget = (props: ICompletedVerificationsWidgetProps) => (
  <Segment>
    ...
    <Container fluid={false} style={{ paddingBottom: '14px' }}>
      {props.completedVerifications.map((ele: IVerificationRequest, i) => <Grid key={i}>
        ...
        <Grid.Row columns={2} style={{ paddingTop: '0px', paddingBottom: '0px' }}>
          <Grid.Column>
            Student: {getStudentFullName(ele)}<br/>
            Sponsor: {getSponsorFullName(ele)}<br/>
            <Grid.Row style={{ paddingTop: '14px', paddingBottom: '14px' }}>
              <Button basic compact color={getStatusColor(ele.status)} content={'REOPEN'} attached={'right'}
                      label={{
                        size: 'mini',
                        basic: false,
